...
// will use std::hardware_destructive_interference_size instead of hardcoding it
// here and we'll use C++ attribute syntax.
...
static const struct ggml_type_traits_cpu type_traits_cpu[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_IQ3_XXS] =
        {
            // NOTE: from_float for iq3 and iq2_s was removed because these
            // quants require initialization in ggml_quantize_init
...
#define ggml_cond_wait(c, m)                                                   \
  SleepConditionVariableSRW(c, m, INFINITE, CONDITION_VARIABLE_LOCKMODE_SHARED)
...
struct ggml_threadpool {
  ...
  atomic_int n_graph; // updated when there is work to be done (i.e each graph)
                      // holds graph and active thread counts.
  atomic_int GGML_CACHE_ALIGN n_barrier;
  atomic_int GGML_CACHE_ALIGN n_barrier_passed;
  atomic_int GGML_CACHE_ALIGN
      current_chunk; // currently processing chunk during Mat_Mul, shared
...
#pragma omp barrier
  ...
  while (atomic_load_explicit(&tp->n_barrier_passed, memory_order_relaxed) ==
         n_passed) {
    ggml_thread_cpu_relax();
  }

// exit barrier (full seq-cst fence)
// TSAN doesn't support standalone fence yet, we use a dummy read-modify-write
// instead
...
int ggml_threadpool_chunk_add(struct ggml_threadpool *tp, int value) {
  return atomic_fetch_add_explicit(&tp->current_chunk, value,
                                   memory_order_relaxed);
}
  ...
  if (ggml_is_numa()) {
    ...
    if (fptr != NULL) {
      ...
      if (fgets(buf, sizeof(buf), fptr) &&
          strncmp(buf, "0\n", sizeof(buf)) != 0) {
        GGML_LOG_WARN("/proc/sys/kernel/numa_balancing is enabled, this has "
                      "been observed to impair performance\n");
      }
      fclose(fptr);
    }
  }
...
void ggml_compute_forward_mul_mat(const struct ggml_compute_params *params,
  ...
  // Custom SIMD backend hook - try optimized AVX2/AVX-512 kernels first
  extern int ggml_simd_try_mul_mat(const struct ggml_compute_params *params,
                                   struct ggml_tensor *dst);
...
#define MMID_MATRIX_ROW(row_id, i1)                                            \
  matrix_rows[(row_id) * ids->ne[0] * ids->ne[1] + (i1)]
...
static void ggml_compute_forward_mul_mat_id_one_chunk(
    struct ggml_tensor *dst, const struct ggml_tensor *src0,
    const struct ggml_tensor *src1, const struct ggml_tensor *ids,
    const int64_t cur_a, const int64_t ir0_start, const int64_t ir0_end,
    const int64_t ir1_start, const int64_t ir1_end, const char *src0_cur,
    const struct mmid_row_mapping *matrix_rows, const size_t row_size,
    const bool src1_cont, const void *wdata) {
  ...
  for (int64_t iir1 = ir1_start; iir1 < ir1_end; iir1 += blck_1) {
    for (int64_t iir0 = ir0_start; iir0 < ir0_end; iir0 += blck_0) {
      for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir1_end; ++ir1) {
        ...
        // to handle this
        const char *src1_col =
            (const char *)wdata + (src1_cont || src1->type != vec_dot_type
        ...
        for (int64_t ir0 = iir0; ir0 < iir0 + blck_0 && ir0 < ir0_end; ++ir0) {
          vec_dot(ne00, &tmp[ir0 - iir0], 0, src0_cur + ir0 * nb01, 0, src1_col,
                  0, 1);
        }
...
ggml_compute_forward_mul_mat_id(const struct ggml_compute_params *params,
  ...
  struct mmid_row_mapping *matrix_rows = // [n_as][ids->ne[0]*ids->ne[1]]
      incr_ptr_aligned(&wdata_cur,
                       n_as * ids->ne[0] * ids->ne[1] *
                           sizeof(struct mmid_row_mapping),
                       sizeof(int64_t));
  ...
  GGML_ASSERT(params->wsize >=
              (size_t)((char *)wdata_cur - (char *)params->wdata));
  ...
  for (int cur_a = ith; cur_a < n_as; cur_a += nth) {
    atomic_int *current_chunk_ctr =
        (atomic_int *)(atomic_current_chunk + cur_a);
...
static bool ggml_thread_apply_priority(int32_t prio) {
  // Note that on Windows the Process Priority Class must be updated in order to
  // set Thread priority. This is up to the applications.
...
// Tell Windows that this thread should not be throttled (needs its own CPU
// core). Newer Windows 11 versions aggresively park (offline) CPU cores and
// often place all our threads onto the first 4 cores which results in terrible
// performance with n_threads > 4
    ...
    if (!SetThreadInformation(GetCurrentThread(), ThreadPowerThrottling, &t,
                              sizeof(t))) {
...
struct ggml_cplan ggml_graph_plan(const struct ggml_cgraph *cgraph,
  ...
  if (threadpool == NULL) {
    // GGML_PRINT_DEBUG("Threadpool is not specified. Will create a disposable
    // threadpool : n_threads %d\n", n_threads);
  }
  if (n_threads <= 0) {
    ...
    if (!ggml_cpu_extra_work_size(n_threads, node, &cur)) {
      ...
      case GGML_OP_MUL_MAT: {
        const enum ggml_type vec_dot_type =
            type_traits_cpu[node->src[0]->type].vec_dot_type;
...
static thread_ret_t ggml_graph_compute_thread(void *data) {
  ...
  struct ggml_compute_params params = {
      /*.ith       =*/state->ith,
      /*.nth       =*/atomic_load_explicit(&tp->n_graph, memory_order_relaxed) &
          GGML_THREADPOOL_N_THREADS_MASK,
...
// instead
#ifdef GGML_TSAN_ENABLED
  atomic_fetch_add_explicit(&state->threadpool->n_graph, 0,
                            memory_order_seq_cst);
#else
  atomic_thread_fence(memory_order_seq_cst);
#endif
...
ggml_graph_compute_poll_for_work(struct ggml_compute_state *state) {
  ...
  // This seems to make 0 ... 100 a decent range for polling level across modern
  // processors. Perhaps, we can adjust it dynamically based on load and things.
  const uint64_t n_rounds = 1024UL * 128 * threadpool->poll;

  for (uint64_t i = 0; !ggml_graph_compute_thread_ready(state) && i < n_rounds;
       i++) {
...
static void ggml_graph_compute_kickoff(struct ggml_threadpool *threadpool,
  ...
  // We need the full seq-cst fence here because of the polling threads (used in
  // thread_sync)
...
ggml_threadpool_new_impl(struct ggml_threadpool_params *tpp,
  ...
  for (int j = 0; j < tpp->n_threads; j++) {
    ggml_thread_cpumask_next(tpp->cpumask, workers[j].cpumask, tpp->strict_cpu,
                             &cpumask_iter);
  }
  ...
  for (int j = 1; j < tpp->n_threads; j++) {
    ...
    int32_t rc =
        ggml_thread_create(&workers[j].thrd, NULL,
                           ggml_graph_compute_secondary_thread, &workers[j]);
    GGML_ASSERT(rc == 0);
  }
  ...
  if (!threadpool->pause) {
    // Update main thread prio and affinity at the start, otherwise we'll do it
    // in resume
    ggml_thread_apply_priority(threadpool->prio);
    if (ggml_thread_cpumask_is_valid(threadpool->workers[0].cpumask)) {
      ggml_thread_apply_affinity(threadpool->workers[0].cpumask);
...
#pragma omp single
      {
        // update the number of threads from the actual number of threads that
        // we got from OpenMP
        n_threads = omp_get_num_threads();
        atomic_store_explicit(&threadpool->n_graph, n_threads,
                              memory_order_relaxed);
      }
...
void ggml_cpu_fp16_to_fp32(const ggml_fp16_t *x, float *y, int64_t n) {
  ...
  for (; i < np; i += step) {
    ...
    vfloat16m2_t ax1 =
        __riscv_vle16_v_f16m2((const _Float16 *)x + i + epr, epr);
...
void ggml_cpu_bf16_to_fp32(const ggml_bf16_t *x, float *y, int64_t n) {
  ...
  for (; i + 15 < n; i += 16) {
    _mm512_storeu_ps(
        ...
        _mm512_castsi512_ps(_mm512_slli_epi32(
            _mm512_cvtepu16_epi32(_mm256_loadu_si256((const __m256i *)(x + i))),
            16)));
  }
#endif
  for (; i + 7 < n; i += 8) {
    _mm256_storeu_ps(
        ...
        _mm256_castsi256_ps(_mm256_slli_epi32(
            _mm256_cvtepu16_epi32(_mm_loadu_si128((const __m128i *)(x + i))),
            16)));
  }
  ...
  for (; i < np; i += step) {
    ...
    vbfloat16m2_t ax1 =
        __riscv_vle16_v_bf16m2((const __bf16 *)x + i + epr, epr);
...
void ggml_cpu_init(void) {
  ...
  if (is_first_call) {
    // initialize GELU, Quick GELU, SILU and EXP F32 tables
    {
      ...
      //     // set the wait policy to active, so that OpenMP threads don't
      //     sleep setenv("OMP_WAIT_POLICY", "active", 0)
      // }

      if (!getenv("KMP_BLOCKTIME")) {
        // set the time to wait before sleeping a thread
        // this is less aggressive than setting the wait policy to active, but
        // should achieve similar results in most cases
#ifdef _WIN32
